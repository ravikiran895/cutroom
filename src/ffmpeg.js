 ...
 */
export async function exportMp4({ clips, audio = [], sources, useOriginal, width, height, quality, onProgress }) {
  ...
  audio.forEach((a, i) => {
    ...
    fc.push(
      `[${si}:a]atrim=${r(a.in)}:${r(a.out)},asetpts=PTS-STARTPTS,` +
      `volume=${a.vol ?? 1},adelay=${ms}:all=1[ac${i}]`
    );
    mixLabels.push(`[ac${i}]`);
  });
